...
df = df.sort_index()
df = df.applymap(lambda x: float(x.split(':')[1]) if x != "N" else np.nan)
cmap_name = "tab20c"
...
plt.title('Bottlenecks severity based on maximum capacity availability during a day', fontsize=14)
