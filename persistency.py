...
# Calculate the difference between the values before and after the hyphen
df_diff = df.applymap(lambda x: float(x.split(':')[1]) - float(x.split(':')[0]) if x != "N" else np.nan)

# Use a suitable colormapa
cmap_name = "tab20c"

# Create a figure and subplot for the difference values
fig, ax = plt.subplots(figsize=(12, 10))
im_value = ax.imshow(df_diff, cmap=cmap_name, interpolation='none', aspect='equal')
...
plt.title('Bottlenecks persistency based on the flow variability during a day', fontsize=10)
